                ...
                <div className="relative w-auto my-6 mx-auto max-w-3xl">
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        <div className=" items-start justify-between p-5">
                            ...
                            <h4 className="block text-xl text-center font-semibold">Modify Transaction</h4>
                        </div>
                        <div className="flex row p-6 ...">
                            <form ref={props.reference} onSubmit={props.onSubmit}>
                                ...
                                <div className="form-layer w-6/6">
                                    ...
                                    <input
                                        className="px-2.5 py-1.5 border border-gray-light rounded-full w-4/6 m-4"
                                        type="text"
                                        id="concept"
                                        name="concept"
                                        defaultValue={props.concept}
                                        required
                                    />
                                </div>
                                <div className="form-layer">
                                    <label className="font-bold uppercase" htmlFor="transaction-type">Type of Transaction: </label>
                                    <span className="font-bold uppercase">{props.type === true ? 'Ingress' : 'Egress'}</span>
                                </div>
                                <div className="form-layer w-6/6">
                                    <label className="font-bold uppercase" htmlFor="Amount">Amount</label>
                                    <input className="px-2.5 py-1.5 border border-gray-light rounded-full w-3/6 m-4" type="number" id="amount" name="amount" defaultValue={props.amount} required/>
                                    <span className="w-3/6 font-bold"> USD</span>
                                </div>
                                <div className="form-layer">
                                    <label className="font-bold uppercase" htmlFor="category">Category</label>
                                    <input className="px-2.5 py-1.5 border border-gray-light rounded-full w-4/6 m-4" type="text" id="category" name="category" defaultValue={props.category} required/>
                                </div>
